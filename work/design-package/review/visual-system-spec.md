...
> Active revision: 7 | Completed stage: `design_system` + Stage 12 final patch round CR-08 | Sources: selected concept `Surface Thread`; interaction r10; PM r5; UXR r4; Stage 12 attempt D

## 1. Direct Description of Outputs

本修订保留通过审查的“表面光轨”，并将其落为可消费的视觉 token、窗口结构、七个核心组件完整结构块、材料/深度、数据语义与资产契约。
...
|---|---|---|---|---|---|---|---|---|
| V1 表面光轨 Surface Trace | “实物是纸，标尺是光学墨迹”；只在当前决策需要的表面上留下几何 | 首见仍是现实墙/桌/地；命中位置仅出现一个带法线短线的空心点；放两点后亮蓝半透明线沿表面生长，数值在中点上方小幅离面 | Shared Space hub 只是入口/历史；Stage Mixed 中没有大主面板，仅有就近测量实体和视野右下辅助命令（附着形式待 Stage 9） | environment=0; plane grid=+1mm; ruler=+3mm along normal; ticks=+4mm; endpoint=+6mm; billboard value=near user but angularly adjacent, never over endpoint | 1 实物边缘 → 2 命中点/当前线 → 3 当前数值/可信状态 → 4 次级刻度 → 5 命令 | hollow-dot=preview; filled dot=pinned; dashed+text=degraded; growing line=commit continuity; palm clear opens explicit Dialog | 高：位置/距离/尺度直接共位，不建第二个 3D 副本 | 低：量测场景没有卡片网格；历史才使用单列阅读面 |
...
| Market differentiation foil | avoids C1/C2 screen mediation and C3 full-room overlay | recreates screen-mediated tool console | visually novel but drifts toward decorative XR |
...
- **Selected direction / approved visual reference**: **V1 表面光轨 Surface Trace**. It is the only option where the real edge remains the first view, the trust state is adjacent to its source, and the distinctive spatial value is achieved without a dashboard or decorative floating constellation. Structured design-effect review `DER-20260815-VIS-R1-ISO-01` passed on visual r1 + interaction r3 + PM r5 + UXR r4.
- **Rejected V2**: despite excellent 2D readability, it turns the real surface into supporting content, violates the low-occlusion premise, and visually converges on the screen-mediated competitor baseline observed in UXR C1/C2.
...
| reviewedDirection | V1 Surface Trace |
...
| invocationId | DER-20260815-VIS-R1-ISO-01 |
| evidenceRebuilt | yes |
| reviewedRevision | visual r1; interaction r3; PM r5; UXR r4 |
| verdict | pass |
...
Grid: one column, rows 48/56/1fr/72dp-equivalent, 16dp gaps. All window regions map to `HubWorkspace`; decorative ruler token is a render element inside its block, not a second component. Large uses 4-column modes/list+detail; default 2×2/single detail; compact one-column/internal scroll.
...
│ R4 ┊ Large-only receipt / Coachmark (one)   ┊ │
...
|---|---|---|
| Large | 0.91×0.53m face | `C-HUB` max; 4-mode row or history 60/40, within 0.96×0.58m |
| Regular | 0.67×0.39m face | default; 2×2 modes / one-column history, within 0.72×0.44m |
| Compact | 0.51×0.29m face | min; one-column modes, internal scroll, within 0.56×0.34m |
...
| `records.items[].screenshotUri` | thumbnail/link | `无截图` | display-only |
...
| `session.screenshotUri` | shot status/payload | `无截图` | display-only |
...
| `receipt.uri` | success attachment link | no link; never fake saved | semantic |
...
| `ScreenshotReceipt` | async | StatusReceipt `receipt.*` | pending/success/error | binding present |
...
| T07 accept/undo/restart | actionable | Control Undo/Finish/Clear; Ruler result | covered |
| T08 undo/request clear/cancel/confirm | actionable | Control `ctl-undo/clear`; DecisionDialog actions | covered |
| T09 save/retry/cancel | actionable | Control `ctl-save` triggers TR-12→D1; RecordComposer input/save/cancel + StatusReceipt Retry complete the flow | covered |
| T10 capture/retry/dismiss | actionable | Control `ctl-capture` + StatusReceipt actions | covered |
...
- Current deliverable: complete visual/design-system contract r7.
