...
> Source identity: `spatial-ruler-design-run-4-20260816` | Active artifact revision: **2** | Sources: PM r1 intent + UXR r1 research. No prior-run verdict or revision is active.

> Role: `product_strategist` | Active revision: 2 | Workflow stages: `intent` → `quality_contract` | Sources: 用户原始需求; `uxr-research-report.md` r1; local Spatial SDK 0.13 API references recorded in UXR source register
>
...
1. **O1 可放置**：用户只能在可信平面命中上提交点；降级命中只允许显示虚线预览，`canCommitPoint=false`、`canComplete=false`，不得生成完成数值。移回新鲜可信平面并连续稳定命中后恢复提交。
2. **O2 四模式完成**：直线、连续、矩形面积、垂直高度各有进入、标点、撤销、完成、异常、退出流程。
3. **O3 贴合且可信**：线/点/刻度与检测平面共位；tracking loss、无可信 plane、异面点、地面未就绪或 freshness 超阈值时冻结旧几何为“非当前”，禁止完成/保存当前值。稳定恢复为：提示用户把射线移到已识别表面 → 连续新鲜命中 → 重新验证既有点/必要时重放点 → 才恢复完成。
4. **O4 值可读可换算**：显示值面向用户，原始米制数据在 cm/m/in 切换中保持等价。
5. **O5 可恢复**：任何模式可撤销最后一点；张掌只请求清除，经 Dialog 显式确认后执行。
6. **O6 可留存**：Room 在离线状态仍可创建/重命名/删除记录；写入成功必须返回 recordId 和可见 receipt，失败必须保留草稿并允许重试。截图请求必须建立 currentSession 所有权的 queue item；成功返回可打开的 `content://` URI 并只关联当前记录/会话，权限拒绝、队列失败、媒体写入失败不得伪装成功，必须保留测量并提供 Retry/Dismiss。
...
根据 UXR §3A “Our differentiation opportunities”：吸收 C1 的两点快速任务/近似性告知、C2 **已由 App Store 文案核验的功能覆盖**、C3 的扫描质量引导/可撤销/留档机会；避免手机屏幕对齐负担、把未经观察的工具盘密度当作事实、以及过重房间建模流程。差异必须由戴显共位、表面质量可见、低遮挡尺标与会话级空间连续性证明；不复用三个竞品的布局/状态/组件/视觉。
...
|---|---|---|
| R1 直线测量 / O1 degraded guard | two-point marking + `hit.trust=fresh` guard; degraded preview has `canCommitPoint=false` and `canComplete=false` | 用已知 1 m 标准物多位姿比对；注入 degraded hit 后断言无完成值/无记录写入；恢复连续 fresh hit 后才能完成 |
| R2 贴平面 / O3 tracking guard | plane-aware hit + surface lock + `tracking.ageMs/plane.trust` fail-closed | 墙/桌/地观察法线偏差；注入 tracking loss/no trusted plane 后断言完成和保存禁用、旧值标为非当前；按“移回可信表面→连续 fresh→重验点”恢复 |
...
| R7 测量记录 / O6 offline Room | naming/save/history + offline repository states | 断网创建记录，成功断言 recordId/name/rawMeters/unit/timestamp 且重启可读；注入 Room error 时草稿保留、receipt=error、Retry 成功前不显示“已保存” |
| R8 截图保存 / O6 queue+URI | current-session capture queue + ownership + receipt | 请求后 queue item 带 sessionId；成功 URI 必须是可点击 `<a href>`/可打开 `content://` 且只写回当前 session/record；权限拒绝、queue error、media write error 均显示 error、保留测量、Retry/Dismiss 可达 |
...
| R14 安全退出（O7） | `T-EXIT-REQUEST` → exit-confirm/cancel → close Stage → Shared Space hub; system Back/interruption recovery | Preview 分别触发完成、系统 Back、中断三条路径并确认回到 Hub；真机再验 Stage 关闭与 Shared Space 返回 |
| R15 历史持久语义（O8） | measurement record payload stores numeric result/name/unit/timestamp/current-session screenshot URI only; `worldPoseRestored=false` | 重启后断言记录字段可读、截图 URI 可解析；负向断言历史不会创建锚点/几何或恢复世界姿态 |
...
| Requirements traceability | O1–O8 与全部 mandatory requirements 均映射；O7→R14，O8→R15 | §7.1 O1–O8；§8 R1–R15 | pass |

| Field | Value |
|---|---|
| minimumCompletenessGate | pass |

> This run keeps exact API annotations separate from manager-workflow inference and treats all device behavior as pending downstream verification.

## 10. Delivery and Recipients

- **Current deliverable**：frozen intent + quality contract revision 2.
