...
> Source identity: `spatial-ruler-design-run-4-20260816` | Active revision: **3** | Completed stage: `design_system` CR-12A | Sources: approved V1 Edge Whisper visual r1; interaction r7; PM r2; UXR r1; Stage 12 Attempt A findings.
...
│ R3 ┊ secondary row OR capture StatusReceipt ┊ │
│ R4 ┊ Large-only capture receipt             ┊ │
│    └╌ DecisionDialog / ordinary RecordComposer ╌│
└───────────────────────────────────────────────┘
```

Normal composition is readiness + primary + secondary. **Capture** receipt composition keeps primary/stable-exit controls and replaces the secondary row at Compact/Regular; its 96dp arithmetic remains `204≤216`, `228≤276`, `236≤396`. D0/D2/D3 Dialog temporarily spans safeContent with modal focus. D1 `RecordComposer` also uses the host body but remains ordinary, non-modal in-window content and owns its inline write states; it never instantiates StatusReceipt. Compact preserves 56dp targets.
...
| `hub-record-shot-link` | 打开截图 | link | `records.selected.screenshotUri` | history only; visible iff selected-id/owner-record relation is valid; current session is irrelevant |
...
| `records.selected.recordId` | selected row identity | missing/nonpositive → no selected detail/action | semantic |
| `records.selected.screenshotUri` | `hub-record-shot-link.href` + selected thumbnail | hide link + `无截图` for null/malformed/owner mismatch; do **not** compare current session | semantic ownership |
| `records.selected.screenshotOwnerRecordId` | historical URI ownership guard | must equal `records.selected.recordId`; mismatch hides link and reports corrupt association | semantic |
| `records.selected.screenshotOriginSessionId` | audit detail only; never a current-session guard | `来源会话未知`; link remains legal when owner record matches | display-only |
| `records.selectedId` | rename/delete action scope + selected relation | must equal `records.selected.recordId`; otherwise clear selection and disable actions | semantic |
...
| `surface-recovery` | 正在重新确认 3/5 | progress+text | `recovery.consecutiveFreshFrames`, `recovery.freshDurationMs` | E0 only; never implies ready before both guards pass |
...
| `recovery.consecutiveFreshFrames` | recovery progress/guard | unknown or invalid frame → 0 | semantic |
| `recovery.freshDurationMs` | recovery progress/guard | unknown or invalid frame → 0ms | semantic |
...
Combined current-hit guard: `meshRequired = placement.sourceKind == mesh_assisted`; `canCommit = planeFresh && rayHit.valid && placement.sourceKind known && (!meshRequired || (mesh.availability==available && mesh.hitConfidence>=projectMeshThreshold))`. Commit atomically copies the six provenance fields. After any stale/unknown frame, `recoveryStable = consecutiveFreshFrames>=5 && freshDurationMs>=250`; every invalid/unknown/source mismatch resets both counters to zero. `canComplete = recoveryStable && points.isNotEmpty && points.all(freshRevalidate(point.provenance)) && modeGeometryValid`; it never substitutes current hit for a point snapshot. Thresholds 5 frames/250ms and `projectMeshThreshold=0.70` are project starting assumptions pending device calibration, not platform constants. Unknown fails closed and keeps Undo/Back available.
...
| `points[].provenance.sourceKind` | per-point `plane_only` / `mesh_assisted` Finish guard | unknown → fail closed; offending point square | semantic |
| `points[].provenance.planeAnchorId` | same-plane/revalidation key; never shown raw | unknown/missing anchor → fail closed | semantic |
| `points[].provenance.meshAnchorId` | required only when sourceKind=mesh_assisted | missing for mesh-assisted → fail closed; plane-only may be null | semantic |
| `points[].provenance.confidenceAtCommit` | immutable commit snapshot + revalidation comparison | unknown/out of [0,1] → fail closed | semantic |
| `points[].provenance.committedAt` | freshness/ordering snapshot | missing → fail closed at Finish | semantic |
| `points[].provenance.frameId` | correlation to the sensing frame used at commit | missing/mismatched frame → fail closed | semantic |
...
Point commit schema is immutable per point: `{worldMeters, provenance:{sourceKind, planeAnchorId, meshAnchorId?, confidenceAtCommit, committedAt, frameId}}`. `Finish` revalidates **every** point against a fresh anchor observation: anchor exists; frame transform is resolvable; age is within `freshThreshold`; current confidence meets the source-specific threshold; and same-plane/fit rules pass. `mesh_assisted` additionally requires matching `meshAnchorId` and fresh mesh availability. Any unknown/missing/mismatch fails closed, keeps Undo/Back active, and names the offending point. Current-ray evidence can preview but can never substitute for a committed point snapshot.
...
| `exit.priorState` | D2 Cancel/system Back exact return target | missing/invalid → M0 without clearing any retained session data; announce recovery fallback | semantic |
...
- `discardExit`: confirms unsaved discard and Stage close to H0; Cancel and system Back restore the exact allow-listed `exit.priorState` (`M-L/M-P/M-A/M-H/M2/E0`) with points/result intact; corrupt/missing value falls back to M0 without clearing retained data.
...
This is an ordinary in-window focused form, not a Dialog, Sheet or attachment. Grid is one column and `RecordComposer` itself owns pending/error/retry/success; no second receipt overlays or replaces it. Worst-case error is a one-line visible reason in C plus full accessible detail. Compact rows 24/56/32/56dp, 6dp gaps, 8dp top/bottom padding: `24+56+32+56+18+16=202dp ≤216dp`. Regular rows 36/56/48/56dp, 8dp gaps, 16dp top/bottom: `36+56+48+56+24+32=252dp ≤276dp`. Large uses 36/56/48/56dp, 12dp gaps, 20dp padding: `36+56+48+56+36+40=272dp ≤396dp`. At 1.5× text, A/C wrap is prohibited; copy is shortened with accessible full text, so hit rows remain 56dp and all tiers fit. In error, D is `[取消][重试]`; in pending, Cancel remains enabled and the right action is disabled `正在保存`; on success, D remains visible for the ≥300ms observable receipt interval before returning M2.

**Anatomy · Sizing (sizing)**

| Tier | Width × Height | Window tier / fit |
|---|---|---|
| Regular | 672×252dp | default `safeContent` 672×276; worst error 252dp |
| Compact | 512×202dp | min `safeContent` 512×216; worst error 202dp |
| Large | 720×272dp | max `safeContent` 912×396; worst error 272dp |
...
| radius | 32dp | radius.l |
| padding | 16dp regular; 8dp compact; 20dp large | calibrated within safeContent |
| gap | 8dp regular; 6dp compact; 12dp large; 16dp action columns | scale plus explicit Compact exception |
...
| `record-write-status` | 正在保存/已保存/保存失败 | shape+label | `recordWrite.state`, `recordWrite.recordId`, `recordWrite.reason` | inline truth; success only for positive id |
| `record-retry` | 重试 | button | `recordWrite.retryable` | error replaces Save, Cancel remains |
...
| `session.screenshotUri` | `record-shot` status + `record-shot-link.href` | `无截图`; hide link; active foreign-session capture rejected before persistence | semantic ownership |
| `capture.currentSessionId` | D1 screenshot-link ownership guard before record creation | mismatch with `session.sessionId` → hide/reject link | semantic |
| `recordWrite.state` | form state/receipt | preserve draft on error | semantic |
| `recordWrite.recordId` | success label and TR-13 guard | nonpositive/missing → error; stay D1 | semantic |
| `recordWrite.reason` | C-row short error + accessible detail | `保存失败，请重试` | display-only |
| `recordWrite.retryable` | Retry visibility/enabled | false | semantic |
| `recordWrite.intentId` | Room unique idempotency key for submit/retry | missing → Save disabled; never generate a new key on Retry | semantic |
| `recordWrite.cancelRequested` | pending Cancel state and resolution copy | false; stay D1 until rollback/commit outcome | semantic |
...
| loading | write pending | C=`正在保存`; input and Cancel preserved; right action disabled; pending Cancel sets `cancelRequested` and copy `正在取消` without leaving D1 | none | static under RM | polite live region |
| empty | blank name | field warning; generated-name option | none | none | explicit validation |
| error | DB write fails/nonpositive id | C square+one-line reason; Retry replaces Save, never metadata/input/Cancel | none | MO-07 | full reason announced; draft preserved |
| success | committed Room transaction + `recordId>0` | C diamond+`已保存`; all form rows remain stable | none | MO-07 ≥300ms or one observable render turn | announces record id as saved state, not raw number |
...
`RecordComposer` exclusively owns D1 pending/error/retry/success inline. `StatusReceipt` is not instantiated for record write. `intentId` is generated once when D1 opens and has a Room UNIQUE constraint; Submit and every Retry use the same key. Pre-submit Cancel returns M2. Pending Cancel stays D1 until the transaction reports rollback or committed id: rollback returns M2 unsaved; late positive id renders success ≥300ms then returns M2, so no committed record is hidden. Retry of an already committed intent resolves the same id and cannot duplicate.
...
| derivedFromData | `ScreenshotReceipt`, `CaptureQueueItem`, media-write receipt |
| purpose | State screenshot capture/media-write pending/success/failure truthfully and provide retry/dismiss; never carry record-write state |
...
| `receipt-detail` | 截图可关联当前测量/存储不可用 | caption | `receipt.reason` | capture trust/recovery |
...
| Source path | Target | fallback | kind |
|---|---|---|---|
| `receipt.operation` | capture/media-write message verb | `截图操作` | display-only |
...
| `capture.currentSessionId` | active-capture ownership guard | mismatch with active `session.sessionId` → reject active attachment + error | semantic |
| `capture.attachedRecordId` | post-transaction owner (nullable before record create) | nonpositive/mismatched id → do not expose as historical attachment | semantic |
...
- `screenshotCapture`: Stage capture pending/success/error; success links URI to the active request/session.
- `mediaWrite`: system media-store write pending/success/error for the same capture pipeline; never a Room record-write receipt.
...
| point coordinates | session stable after commit; not persisted in history | Ruler `points[]` | world endpoints in current Stage only | binding present |
| point sourceKind | immutable commit + Finish revalidation | Ruler `points[].provenance.sourceKind` | chooses plane-only/mesh-assisted guard; unknown fails closed | binding present |
| point planeAnchorId | immutable commit + Finish revalidation | Ruler `points[].provenance.planeAnchorId` | anchor/same-plane lookup; raw id not shown | binding present |
| point meshAnchorId | immutable when mesh-assisted | Ruler `points[].provenance.meshAnchorId` | mesh revalidation; required only for mesh-assisted | binding present |
| point confidenceAtCommit | immutable snapshot + Finish | Ruler `points[].provenance.confidenceAtCommit` | compare against fresh current confidence | binding present |
| point committedAt | immutable snapshot + Finish | Ruler `points[].provenance.committedAt` | freshness/order; accessible time | binding present |
| point frameId | immutable snapshot + Finish | Ruler `points[].provenance.frameId` | sensing-frame correlation; raw id hidden | binding present |
...
| `MeasurementRecord.recordId` | persistent write result | RecordComposer `recordWrite.recordId`; Hub `records.items[].recordId` row key/selection | positive-id success gate | binding present |
| `CaptureQueueItem` request/currentSession/record/return | async | StatusReceipt `capture.requestId`, `capture.currentSessionId`, `capture.attachedRecordId`, `capture.returnState` | correlation/active ownership/persisted ownership/routing | binding present |
| current-session/record screenshot URI | async→persistent | Receipt `receipt.uri`; RecordComposer `session.screenshotUri`; Hub `records.selected.screenshotUri` + ownerRecordId | active mismatch rejected; historical link ignores current session and requires selected/owner record equality | binding present |
...
|---|---|---|---|
| T01 enter/stay | actionable | Hub `hub-start` explicit Stage entry; Back stays in hub | covered |
| T02 active mode/unit | actionable | Hub/Control `hub/ctl-mode`, `hub/ctl-unit` | covered |
| T03 ready/wait/reposition | actionable guard | Readiness `surface-*`; Control commit/Finish guards | covered |
| T04 accept candidate/reposition | actionable | Ruler `ruler-preview`; commit or move ray | covered |
| T05 append/block | actionable | Ruler `ruler-points`; ray tap/trigger with atomic provenance snapshot | covered |
| T06 result/block reason | actionable | Control `ctl-finish`; per-point fresh revalidation + Ruler reason | covered |
...
| T09 save/retry/cancel | actionable | Control `ctl-save`→D1; RecordComposer input/save/cancel/retry and inline success; success requires `recordWrite.recordId>0` | covered |
| T10 capture/retry/dismiss | actionable | Control `ctl-capture`; StatusReceipt correlates requestId/currentSessionId and owns `receipt-link`; Retry/Dismiss preserve `capture.returnState` | covered |
| T11 select/rename/delete/open capture | actionable | Hub record list/actions and owner-record guarded screenshot link; H1R/H1D remain in C-HUB | covered; no restore action |
| T12 resume/discard/return | actionable | Control Back + DecisionDialog bound `exit.priorState`; Cancel exact prior, confirm H0 | covered |
...
| Ruler→point provenance | conflicting | offending point square+Undo | `geometry.planeDeviationMeters`, `points[].provenance.*` fresh revalidation |
...
`MeasurementRecord { recordId:Long>0, intentId:String UNIQUE, name:String, mode:Enum, rawMeters:Double?, rawSquareMeters:Double?, displayUnit:cm|m|in, completedAt:Instant, screenshotUri:String?, screenshotOwnerRecordId:Long?, screenshotOriginSessionId:String?, worldPoseRestored:false }`.

Per-point provenance remains only in the active/completed `MeasurementSession`; it is not a Room field and is discarded with the Stage session. Create is one idempotent transaction keyed by UNIQUE `intentId`: validate result and all session point snapshots → insert-or-resolve the same intent and obtain one positive `recordId` → if the current session owns a valid capture, update that same row with URI, `screenshotOwnerRecordId=recordId`, and `screenshotOriginSessionId=currentSessionId` → commit → expose success. A pending Cancel stays D1 until rollback/commit is known; late commit is shown, rollback returns unsaved; Retry reuses the key and cannot duplicate.
...
| `intentId/name/mode/rawMeters/rawSquareMeters/displayUnit/completedAt` | idempotent intent + frozen completed result | exact fields persist offline; unit never rewrites raw meters; no point/provenance/world transform fields | invalid raw value or unknown session point provenance blocks the transaction before persistence; draft remains editable |
...
| `CaptureQueueItem.requestId/currentSessionId/attachedRecordId/returnState` | `(requestId,currentSessionId)` uniquely owns an active request; `attachedRecordId` is null until a Room transaction attaches it | active receipt requires request+session match and routes to exact returnState; historical ownership later uses attached record id, not current session | permission/queue/media failure keeps geometry and offers Retry/Dismiss; mismatched request ignored; mismatched active session rejected only before persistence |
...
| `MeasurementRecord.screenshotUri/OwnerRecordId/OriginSessionId` | URI is copied only from the creating session inside the record transaction; persisted owner is the generated record id | history link legal iff `selectedId==selected.recordId==selected.screenshotOwnerRecordId` and URI is well formed; it remains legal after app/session restart | current-session mismatch is **not** a rejection rule for history; rename keeps all three fields; corrupt owner relation hides link; delete removes only the row association per storage policy |

`receipt.operation` distinguishes `captureQueue` and `mediaWrite`; record writes are never routed through StatusReceipt. Capture success/error copy and retry are operation-specific. A later capture request queues behind the visible receipt and cannot overwrite current ownership.
...
- Current deliverable: complete visual/design-system contract r3, source identity `spatial-ruler-design-run-4-20260816`, synchronized with interaction r7.
