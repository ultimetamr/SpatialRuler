...
> Active artifact revision: **3** · Stage 4 frozen-reasoning repair round 2; source: PM r2, user requirement, official rules, and bounded read-only prior research facts. This fresh run owns the revision identity; old-run CR labels below are provenance notes only.

> Role: `research_analyst` | Active revision: 3 | Workflow stage: `research` + Stage 4 bounded evidence repair EIR-R1/EIR-UXR-01/EIR-CB-01
> Upstream: `pm-requirement-spec.md` r2, 用户原始材料, `knowledge/official-rules.json` v2.2.0 | Observation date: 2026-08-15
...
|---|---|---|---|---|---|---|---|---|
| E-M1 | market | iOS 移动 AR 测量基线已包括两点长度、矩形/面积、高度与截图；AR Ruler 的 iOS listing 另列多点/体积/多单位，Android 范围未核验 | Apple Measure 官方指南；AR Ruler App iOS App Store listing | official | iOS 同任务邻近产品；Android=gap | high for listed iOS features / low for Android | accessed 2026-08-15 | 将已核验功能基线与 PICO 原型任务成功率对比；Android 单独核验；不复用 UI |
...
| C2 | AR Ruler App / iOS（Android 可用性未在本轮核验） | App Store 列出的长度、距离、角度、面积/周长、多点路径、高度、体积、房间扫描、多单位；具体 Android 差异是 gap | **Evidence gap / assumption, low**：商店文案未证明工具切换和点按路径；须录屏式上手计时验证 | **Evidence gap / assumption, low**：商店页未充分证明工具面板密度；须用第一方截图逐屏标注或实机观察 | 功能语义利用平面/尺度/距离；是否充分利用身体/深度是 gap；迁移仅限“工具覆盖”机会 | App Store first-party listing, accessed 2026-08-15: https://apps.apple.com/us/app/ar-ruler-digital-tape-measure/id1326773975；interaction/visual source=none, validation plan as stated |
...
| C2 | 仅吸收 App Store 已核验的模式覆盖、多单位、路径总长、高度/面积需求 | 不从未核验的 interaction/visual gaps 推导工具盘反模式；不把商店精度营销用语当可信度证据 |
...
## 3B. Reproducible Source Register (CR-01)
...
| S-P6a | `C:/Users/Administrator/AppData/Local/PICO/sdk/0.13/agent-vault/api-reference/com.pico.spatial.sense.plane.md` | `PlaneAnchor` near line 14 → `RequiredFullSpace`; `PlaneTrackingManager` near line 180 has no such annotation | 2026-08-15 | local official 0.13 API snapshot; exact fact corrected in CR-03 |
| S-P6b | `C:/Users/Administrator/AppData/Local/PICO/sdk/0.13/agent-vault/api-reference/com.pico.spatial.sense.mesh.md` | `MeshAnchor` near line 13 → `RequiredFullSpace`; `MeshTrackingManager` near line 125 has no such annotation | 2026-08-15 | local official 0.13 API snapshot; exact fact corrected in CR-03 |
| S-P6c | `C:/Users/Administrator/AppData/Local/PICO/sdk/0.13/agent-vault/api-reference/com.pico.spatial.tracking.hand.md` | `HandTrackingData` near line 204 and `HandTrackingProvider` near line 248 → `RequiredFullSpace` | 2026-08-15 | local official 0.13 API snapshot; exact loci added in CR-04 |
...
> Change control: CR-01 only enriched provenance and inference labels. CR-03 corrected exact 0.13 annotations and fact/inference separation. CR-04 removed stale space-state evidence, added hand loci and claim-level magicplan sources. None adds a market-derived UI fact.
...
- **Deliverables**：research evidence + domain model revision 3.
